...
mothur <- data.frame(otu_table(physeq_mothur))
write.csv(mothur, 'mothur.csv')
...
qiime <- data.frame(otu_table(physeq_qiime))
write.csv(qiime, 'qiime.csv')
...
MG <- data.frame(otu_table(physeq_MG))
write.csv(MG, 'MG.csv')
