...
legend(x='top', legend = c("MG-RAST", "mothur", "QIIME2", 'Even' , "Staggered"), pch = c(15,15,15,15,12), 
...
tiff('sequence_error_MG.TIF', width = 500, height = 400)
ggplotRegression <- function(fit){
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    ...
    labs(title = paste('MG-RAST', 
                       "Adjusted R2 = ",signif(summary(fit)$adj.r.squared, 5)))
}

ggplotRegression(lm(Expected ~ Abundance, data = compare_mg))
dev.off()

compare_mot <- changing_false_negatives(compare_mot)
tiff('sequence_error_mot.TIF', width = 500, height = 400)
ggplotRegression <- function(fit){
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point(color = "#ffb31a", size = 3) + 
    geom_abline(slope =1, intercept = 0, color='blue') +
    geom_abline(slope =0, intercept = 0, color='black') +
    geom_vline(xintercept = 0, color='black') +
    stat_smooth(method = "lm", col = "red") +
...
ggplotRegression(lm(Expected ~ Abundance, data = compare_mot))
dev.off()

compare_qim <- changing_false_negatives(compare_qim)
tiff('sequence_error_qim.TIF', width = 500, height = 400)
ggplotRegression <- function(fit){
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point(color = "#5c5c8a", size = 3) + 
    geom_abline(slope =1, intercept = 0, color='blue') +
    geom_abline(slope =0, intercept = 0, color='black') +
    geom_vline(xintercept = 0, color='black') +
    stat_smooth(method = "lm", col = "red") +
...
ggplotRegression(lm(Expected ~ Abundance, data = compare_qim))
dev.off()
