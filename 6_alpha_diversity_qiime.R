...
tiff("alphadiv_qiime_subsample.TIF", width = 600, height = 400)
p <- ggplot(rich, aes(x=Subsample, y=Observation, fill=Subsample)) +
  geom_boxplot() + ylab("Counts")
c <- p + facet_wrap(~Index, scales="free") + scale_fill_manual(values = c("#512465", "#197C75", "#DAF7A6"))
dev.off()

tiff("alphadiv_qiime_rarefy.TIF", width = 600, height = 400)
